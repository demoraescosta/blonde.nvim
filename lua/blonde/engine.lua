...
function M.generate()
  ...
  if type(options.colors) == 'string' then
    local colors_ok, colors_or_err = pcall(require, 'blonde.colors.' .. options.colors)
  ...
  if type(options.highlights) == 'string' then
    local highlights_ok, highlights_or_err = pcall(require, 'blonde.highlights.' .. options.highlights)
...
function M.get_color(group_name, color, colors)
  ...
  if type(color) ~= 'string' then
    return false,
      error.throw(
        'blonde.engine.get_color(' .. group_name .. ', ' .. color .. ', ... ) -- Error: color must be a string'
      )
  end
